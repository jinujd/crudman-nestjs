...
export class CrudmanService {
  ...
  async list(section: string, req: any, res: any) {
    ...
    const orm = this.getOrm(actionCfg)
    if (!sectionCfg || !orm) {
      const throwOn = CrudmanRegistry.get().getThrowOnError()
      if (throwOn) {
        const { BadRequestException } = require('@nestjs/common')
        throw new BadRequestException({ success: false, errors: [{ message: 'Invalid section' }] })
      }
      return this.send(res, { success: false, errors: [{ message: 'Invalid section' }] })
    }
  ...
  async create(section: string, req: any, res: any) {
    ...
    if (combinedErrors.length) {
      ...
      if (throwOn) {
        const { BadRequestException } = require('@nestjs/common')
        throw new BadRequestException({ success: false, errors: combinedErrors })
      }
