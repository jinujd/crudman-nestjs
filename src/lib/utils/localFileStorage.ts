...
export class LocalDiskAdapter implements FileStorageAdapter {
  ...
  async save(input: FileStorageSaveInput): Promise<FileStorageSaveResult> {
    ...
    if (input.buffer) await fs.writeFile(abs, input.buffer)
    else if (input.base64) await fs.writeFile(abs, Buffer.from(stripDataUrl(input.base64), 'base64'))
    else throw new Error('LocalDiskAdapter.save requires buffer or base64')
